...
DROP TABLE IF EXISTS users, profiles, states, areas, crags, walls, routes, todos, ticks, shared_images, crag_comments, route_comments, comment_crag, comment_route CASCADE;
...
CREATE TABLE todos (
	...
	CONSTRAINT FK_route_todo FOREIGN KEY (route_id) REFERENCES routes(id) ON DELETE CASCADE
...
CREATE TABLE ticks (
	...
	CONSTRAINT FK_route_tick FOREIGN KEY (route_id) REFERENCES routes(id) ON DELETE CASCADE
);
